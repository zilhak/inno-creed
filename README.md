...
| `send_mail` | 텍스트·Markdown·HTML 파일/원문으로 초안 저장·검증 후 발송(첨부 지원, 미지정 시 본인) |
| `save_mail_draft` | 임시보관함에 저장만(발송 안 함, 첨부 지원) |
| `list_mail_drafts` | 임시보관함 조회 — 저장한 초안 확인, 항목의 `muid`가 후속 조회 키 |
| `preview_mail_draft` | 웹·구버전 초안의 원본 HTML·내용을 미리 보고 발송 기준점 등록(발송 안 함) |
...
- **전 OS 공통으로 확장 프로그램이 정식 경로**입니다. 세션쿠키·파일잠금·`v20` 암호화·키체인/키링 권한 문제를 전부 우회합니다. native host 등록은 서버 기동 때 자동으로 되므로, 사람이 하는 일은 확장 zip을 받아 브라우저에 로드하는 것뿐입니다.
...
| **확장 프로그램(전 OS 필수)** | `inno-creed-extension.zip` |
...
- **메일 본문은 `body`·`html_file`·`html` 중 하나만 입력** — 간단한 내용은 텍스트·Markdown, 서식 있는 내용은 기존 HTML 파일을 권장합니다. `html` 원문 호출도 지원합니다. 웹·구버전 초안은 `preview_mail_draft`로 원문을 다시 작성하지 않고 미리 본 뒤 같은 ID로 발송합니다. 본문 누락·중복 입력·빈 내용은 오류이며, 저장 후 본문 보존을 검증합니다. 새 스키마는 바이너리 교체 후 MCP 재연결로 반영됩니다.
