...
| `mail014A01` → `mail014A14` → 검증 → 초안 발송 | 메일 작성·검증 후 발송(첨부 지원) | `send_mail` |
| `mail014A01` → `mail014A14` → `mail014A01`(draft) | 메일 임시저장·본문 대조(**발송 아님**) | `save_mail_draft` |
| `mail003A01` → `mail014A01`(draft) | 기존 초안 원문 미리보기·본문 기준점 등록(**발송 아님**) | `preview_mail_draft` |
...
`send_mail`과 `save_mail_draft`는 아래 입력 중 **정확히 하나**를 받는다. 알 수 없는 인자,
본문 누락·빈 값·입력 중복은 세션 확보/첨부 업로드/저장/발송 전에 오류로 반환한다.

| 입력 | 용도 |
|---|---|
| `body` | 일반 텍스트·Markdown. HTML 변환·줄바꿈·서명은 도구가 처리 |
| `html_file` | MCP 서버 머신의 UTF-8 HTML 파일 절대경로. 원본 서식을 그대로 읽음 |
| `html` | 기존 호출 호환용 HTML 원문. 가능하면 파일이나 기존 초안 경로를 우선 사용 |

Agent가 HTML을 다시 작성할 필요가 없도록, 서식 있는 메일은 기존 HTML 파일·템플릿의
`html_file` 또는 웹 초안의 `preview_mail_draft(draft_muid)`를 우선 사용한다.
파일 경로의 상대 이미지·CSS 파일은 자동 업로드하거나 내장하지 않는다. HTML 파일의 서식은
보존하지만 아마란스나 수신 메일 클라이언트가 지원하지 않는 기능까지 보장하지 않는다.
도구는 원문 HTML을 유지하고 등록 서명을 body 안에 넣는다. 실측에서 서버는 head의 style 블록을
제거하고 base64 이미지를 서버 URL로 변환했다. 이처럼 구조가 달라지는 신규 저장은 검증 오류로
중단한다. 인라인 스타일과 이미지 URL을 사용하는 메일용 HTML을 권장하며, 남은 초안을 사용할
때는 preview_mail_draft로 현재 형상을 확인받는다.

- `save_mail_draft`: 저장 후 A01(draft)로 HTML을 다시 읽어 작성 본문과 텍스트를 대조한다.
  HTML 입력은 추가로 파싱한 HTML 구조도 비교해 스타일·이미지·링크의 누락/변경을 검출한다.
  서버가 HTML을 재작성해 구조가 달라지면 보수적으로 오류다. 시각적 렌더링 동등성 검사는 아니다.
- 저장/재조회 검증 실패는 **무발송 오류**이며, 이미 저장된 `draft_muid`를 안내한다.
  내용을 확인하지 않고 새 초안을 반복 생성하지 않는다.
- 검증 성공 시 서버 HTML의 SHA-256을 설정 디렉토리 `verified-mail-bodies/`에 계정·초안별로
  기록한다. 본문 원문은 이 기록 파일에 저장하지 않는다.
- **`preview_mail_draft`**: 웹·구버전 초안이나 다른 머신의 초안을 다시 작성하지 않고 사용할 수 있다.
  현재 초안의 제목·수신자·첨부·본문 평문·원문 HTML(`body_html`)을 반환하고 현재 HTML을 기록한다.
  발송/HTML 렌더링/외부 이미지 다운로드는 하지 않는다. 빈 본문·등록 서명만 있는 초안은 거부한다.
  반환 `verification:"existing_draft_snapshot"`은 현재 내용의 기준점이며 **과거 원문과의 대조 검증이 아니다**.
  `ready_for_send:true`도 사람의 승인을 의미하지 않는다. 내용을 확인받은 뒤 발송한다.
- `send_mail_from_draft`: 다시 연 본문 HTML이 기록과 달라졌거나 기록이 없으면 발송하지 않는다.
  `preview_mail_draft`로 다시 확인할 수 있다. 제목·수신자·첨부 변경 이력까지 고정하지는 않는다.
- `send_mail`도 내부적으로 **초안 저장→본문 검증→해당 초안 발송**을 실행한다.
  최종 API 응답을 받지 못하면 "발송 여부 미확정" 오류이며, 자동 재발송하지 말고 보낸메일함을 확인한다.
- 초안 발송의 기존 첨부 제약(콤마 파일명·동명 파일·대용량 첨부)과 최근 20건 조회 제한은 유지된다.

```json
{"subject":"업무 공유", "body":"안녕하세요.\n\n- **확인 사항**: 본문 누락 수정"}
{"subject":"서식 문서", "html_file":"/absolute/path/message.html"}
```

아래 API 실측 기록의 A04는 최종 발송 단계이며, 신규 작성 도구도 A14 검증을 먼저 거친다.
...
- **read-back 검증**: 저장 직후 A01(draft)로 본문을 다시 열어 작성 본문과 텍스트를 대조한다.
  성공 시에만 `verified_by_readback:true`이며, 실패/조회 불가 시 초안 ID를 포함한 오류를 반환한다.
  저장된 초안은 남아 있지만 발송되지 않는다. 상세는 위 "메일 작성·발송 입력 및 본문 검증" 참조.
...
- ⚠️ **빈 기간이면 서버 기본이 좁아 문서를 놓침** → 도구가 최근 약 3개월(`오늘−92일 ~ 오늘`)로 자동 보정.
...
#### `approState` — 취소신청을 낼 수 있는 상태는 하나뿐이다

`at00001` 행의 `approState`가 판별자다. **`1`(결재완료)에만 취소신청을 낼 수 있다.**

| 값 | 관측된 의미 | 취소신청 |
|---|---|---|
| `0` | 상신·결재 진행중 | ❌ `createCancelApplication`이 `resultCode -1 "취소신청이 불가능한 문서입니다.(0)"` → 이 상태는 `cancel_approval`(상신취소)이 맡는다 |
| `1` | 결재완료 | ✅ |
| `2` | 취소신청 레코드 생성 직후(`createCancelApplication` 응답) | — |

⚠️ 서버 거부는 **되돌릴 수 없는 구간에 들어온 뒤에** 온다(`createCancelApplication`이 체인의 6번).
그래서 도구는 1번 조회 결과의 `approState`로 **먼저** 막는다.

⚠️ 조회 범위 주의 두 가지(둘 다 2026-09-15에 실제로 당했다):
- 브라우저 캘린더는 `approStateList:["0","1","4","5"]`만 훑는데 **여기엔 진행중 문서가 안 잡힌다.**
  대상 조회는 전 상태(`"0,1,2,3,4,5"`)로 해야 "취소할 신청이 없다"는 틀린 안내가 안 나온다.
- `at00001` 응답은 `c.call`이 봉투를 벗긴 뒤 **배열 그 자체**다. `resultData`를 한 번 더 꺼내려
  들면 조용히 0건이 된다(에러도 안 난다).
- `idDoc`이 빈 행은 **상신 실패가 남긴 고아 HP 레코드**다(지울 방법이 없다 — 아래 §고아 참조).
  취소 대상이 될 수 없으므로 도구가 거부한다.
...
- `find_person`은 **전사 명부를 조립해 클라이언트에서 거른다**(30분 캐시). 실측 75개 부서 중 인원 있는 곳만 동시 8개로 조회, 첫 호출 약 1.1초 / 이후 약 2ms, 명부 326명.
...
- 전자결재 첨부: **읽기(목록·다운로드)·쓰기(첨부 달아 상신) 모두 구현 완료**.
  ...
  **e2e 실증 완료**(`07 §11.5.4` — 외근41, 첨부 2건 상신→read-back→바이트 대조→취소).
  ⚠️ **문서를 `purge:true`로 지워도 ECM 첨부는 남는다**(`§11.5.5`) — 되돌릴 때 `ecm001A05`로 따로 정리할 것.
