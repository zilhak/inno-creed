...
- 확장 파일 배포: 인스톨러는 `payload/extension/`(전 OS)으로 같이 배포하고, 맨 바이너리로 설치한
  경우에는 릴리즈의 `inno-creed-extension.zip`을 받아 푸는 것이 공식 경로다. **바이너리에 확장
  파일을 내장하지 않는다** — 한때 `include_bytes!`로 넣어 `inno-creed extension` 한 줄로 꺼내게
  해봤지만, 실행 파일이 자기 안의 것을 디스크에 푸는 모양을 만들지 않기로 했다(`installer`가 exe를
  내장하지 않는 것과 같은 결). 다운로드 한 단계를 줄이는 대가로는 크지 않다.
